...
function KingdomSetFounderFaction(faction, kingdom, long_victory)
	...
	local founder = "ai";
	--DF INS
	if not not _G.fkm then
		local level = 1
		if long_victory then
			level = 2
		end
		local fkm = _G.fkm
		if fkm:is_faction_kingdom(faction:name()) then
			fkm._kingdoms[faction:name()]:update_kingdom_level(level)
		end
	end
	--END DF
...
function KingdomResetFounder(kingdom)
	...
	if not not _G.fkm then
		local level = 0
		local fkm = _G.fkm
		if fkm:is_faction_kingdom(KINGDOM_FOUNDERS[kingdom]) then
			fkm._kingdoms[faction:name()]:update_kingdom_level(level)
		end
	end
	--DF END
